%% 这个是计算其他任意视角的视图的
function [cost_h, cost_v] = LocalDepthSPO_owner55_fast_colo2(img_h_RGB, img_v_RGB, NumberOfBins, alpha, nD, opts,uView,vView)
%% 算法实现
%{
    RGB分别处理，划分最bins，
    ...
    uView,vView 分别代表的是行和列的子孔径，如uView=5,vView=1 为5行1列的子孔径
%}
...
for d = 1:nD
    %    test3;
    wh = geth55_weight(alpha , nD , opts,d,uView,vView); % 这里是为了得到权重
